...
export default function ConnectFourArea({
  ...
  // it's the same as the TicTacToeArea
  // just change the names for red and yellow
  // and the gameAreaController?
      ...
      gameStatusText = (
        <b>
          ...
          {gameStatus === 'WAITING_TO_START'
            ? 'Waiting for players to press start' // screen.getByText('Waiting for players to press start'
            : gameStatus === 'WAITING_FOR_PLAYERS'
            ? 'Waiting for players to join' // screen.getByText('Waiting for players to join')
            : 'Over'}
          . {joinGameButton}
        </b>
      );
  ...
  return (
    <Container>
      {gameStatusText}
      {/* Render "Game over" text only when the game status is "OVER" */}
      {/* {gameStatus === 'OVER' && <b>Game over</b>} */}
